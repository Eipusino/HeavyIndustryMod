...
import dynamilize.classmaker.code.annotation.Annotationc;
...
public class ASMGenerator extends AbstractClassGenerator implements Opcodes {
    ...
    protected final Map<String, Fieldc<?>> fieldMap = new HashMap<>();
    ...
    public void visitClass(Classc<?> clazz) {
        ...
        writer.visit(
                ...
                clazz.interfaces().stream().map(Classc::internalName).toArray(String[]::new)
        );
        ...
        for (Element element: clazz.elements()) {
            ...
            if (element instanceof Methodc<?,?> method && method.name().equals("<clinit>")) {
                clinit = method;
                continue;
            }
            element.accept(this);
    ...
    public void visitMethod(Methodc<?, ?> method) {
        ...
        if (!Modifier.isAbstract(method.modifiers())) {
            ...
            for (Localc<?> parameter: method.block().getParamAll()) {
            ...
            for (Map.Entry<String, Localc<?>> entry: localMap.entrySet()) {
        ...
        if (method.name().equals("<clinit>")) {
            for (Map.Entry<String, Object> entry: staticInitial.entrySet()) {
                ...
                Fieldc<?> field = fieldMap.get(entry.getKey());
    ...
    public void visitCodeBlock(CodeBlockc<?> block) {
        ...
        /*if (block.owner().name().equals("<init>") && (!(block.codes().get(0) instanceof Invokec<?>)
                || !((Invokec<?>)block.codes().get(0)).method().name().equals("<init>"))) {
    ...
    public void visitInvoke(Invokec<?> invoke) {
        ...
        Classc<?> type = invoke.method().returnType();
    ...
    public void visitOperate(Operatec<?> operate) {
        ...
        if (operate.leftOpNumber().type() == STRING_TYPE || operate.rightOpNumber().type() == STRING_TYPE) {
            if (operate.opCode() != Operatec.OPCode.ADD)
            ...
            Classc<?> type = operate.leftOpNumber().type();
    ...
    private int getILFD(Classc<?> type) {
    ...
    public void visitArrayGet(ArrayGetc<?> arrayGet) {
        ...
        Classc<?> componentType = arrayGet.array().type().componentType();
    ...
    public void visitArrayPut(ArrayPutc<?> arrayPut) {
        ...
        Classc<?> componentType = arrayPut.array().type().componentType();
    ...
    private static int getAnInt(Classc<?> componentType) {
        int loadType;
    ...
    private static int getType(Classc<?> componentType) {
        int storeType;
    ...
    public void visitReturn(Returnc<?> returnc) {
        if (returnc.returnValue() == null) {
            methodVisitor.visitInsn(Opcodes.RETURN);
        } else {
            Classc<?> retType = returnc.returnValue().type();
            ...
            if (!(returnc.returnValue() instanceof StackElem)) {
                methodVisitor.visitVarInsn(
                        getLoadType(retType),
                        localIndex.get(returnc.returnValue().name())
                );
            }

            castAssign(returnc.returnValue().type(), currMethod.returnType());
    ...
    public void visitNewInstance(NewInstancec<?> newInstance) {
        ...
        if (!newInstance.params().isEmpty() && !(newInstance.params().get(0) instanceof StackElem)) {
            for (Localc<?> local: newInstance.params()) {
                methodVisitor.visitVarInsn(
    ...
    public void visitOddOperate(OddOperatec<?> operate) {
        ...
        Classc<?> type = operate.operateNumber().type();
        int ilfd = getILFD(type);
    ...
    public void visitNewArray(NewArrayc<?> newArray) {
        ...
        if (dimension == 1) {
            Localc<?> len = newArray.arrayLength().get(0);
    ...
    protected void visitAnnotation(AnnotatedElement element) {
        ...
        for (Annotationc<?> annotation: element.getAnnotations()) {
            Classc<?> annoType = annotation.annotationType().typeClass();
    ...
    protected void visitConstant(Object value) {
        if (value == null) {
            ...
            Classc<?> type = ClassInfo.asType(value.getClass());
            methodVisitor.visitFieldInsn(
                    Opcodes.GETSTATIC,
                    type.internalName(),
                    ((Enum<?>)value).name(),
    ...
    protected void castAssign(Classc<?> source, Classc<?> target) {
    ...
    protected void unwrapAssign(Classc<?> target) {
    ...
    protected void wrapAssign(Classc<?> source) {
    ...
    protected static char getTypeChar(Classc<?> primitive, boolean foldInt) {
