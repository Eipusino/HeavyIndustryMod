...
public class YggdrasilTentacle {
	protected FloatSeq points = new FloatSeq(), velocities = new FloatSeq();
	...
	protected static float generationLength = 12f;
	protected static Vec2 v = new Vec2();
	protected static Rect b = new Rect();
	protected static Seq<Building> tseq = new Seq<>(Building.class);
	...
	public void updateTargetPosition(float tx, float ty) {
		...
		for (int i = idx1 + 1; i < size; i++) {
			...
			vr -= Mathf.clamp(Mathm.angleDistSigned(lr, Angles.angle(lx, ly, targetX, targetY)) * 0.02f, -1f, 1f);
			lr += vr;
			//velocities.add(vr, lr);
			//vel[i2] = vr;
			//vel[i2 + 1] = lr;
	...
	public float getProgress() {
		Interp inter = shoot ? HInterps.fastFastSlow : Interp.pow3;
