...
import static heavyindustry.content.HUnitTypes.PHASE;
import static heavyindustry.content.HUnitTypes.STATUS;
	...
	public EmptinessUnitType(String name) {
		...
		class MayflyWeapon extends DataWeapon {
			...
			private MayflyWeapon() {
				...
				bullet = new BulletType() {{
					...
					shootEffect = new MultiEffect(
							HFx.impactBubbleSmall,
							Fx.colorSparkBig
					);
					hitColor = trailColor = HPal.matrixNet;
					hitEffect = HFx.diamondSparkLarge;
					trailEffect = new MultiEffect(HFx.movingCrystalFrag);
					trailChance = 0.4f;
					despawnHit = true;
					speed = 10;
					lifetime = 120;
					...
					public void updateHoming(Bullet b) {
						...
						if (b.time >= homingDelay) {
							...
							if (b.vel.len() >= speed * 0.8f) {
								if (b.timer(3, 3)) HFx.weaveTrail.at(b.x, b.y, b.rotation(), hitColor);
							}
						}
					}
				};
			...
			public void addStats(UnitType u, Table t) {
				...
				Table ic = new Table();
				HStatValues.ammo3(ic, subBullet);
				...
				final float[] farg2 = new float[9];
			{
				...
				public void update(Unit unit, DataWeaponMount mount) {
					...
					if (Mathf.chanceDelta(0.03f*mount.warmup)) {
						...
						Tmp.v1.rnd(3);
						HFx.moveParticle.at(x + Tmp.v1.x, y + Tmp.v1.y, ang, HPal.matrixNet, dst);
					}
				...
				public void draw(Unit unit, DataWeaponMount mount) {
					...
					trail1.draw(HPal.matrixNet, 3*mount.warmup);
					trail2.draw(HPal.matrixNet, 3*mount.warmup);
		...
		weapons.addAll(new Weapon(this.name + "-turret") {{
			...
			class BlastLaser extends LightningBulletType2 { {
				...
				public void createFrags(Bullet b, float x, float y) {
					if (fragBullet != null && fragBullets > 0) {
						Unit[] arr = HUnitSorts.findEnemies(fragBullets, b, fragBullet.range, (us, u) -> {
			...
			bullet = new BlastLaser() {{
				...
				intervalBullet = HBullets.spilloverEnergy.copy();
				...
				laserEffect = new MultiEffect(HFx.laserBlastWeaveLarge, HFx.circleSparkLarge, HFx.impactBubbleBig);
				shootEffect = new MultiEffect(HFx.shootCrossLightLarge, HFx.explodeImpWaveBig, HFx.impactWaveBig, HFx.impactBubble);
				hitEffect = new MultiEffect(Fx.colorSparkBig, HFx.diamondSparkLarge);
				...
				fragBullet = new BlastLaser() {{
					...
					hitEffect = new MultiEffect(Fx.circleColorSpark, HFx.diamondSparkLarge);

					blackZone = false;

					laserEffect = HFx.explodeImpWaveLaserBlase;
					...
					fragBullets = 8;
					fragBullet = HBullets.lightning(128, 32, 62, 5.2f, HPal.matrixNet, b -> {
			...
			protected Teamc findTarget(Unit unit, float x, float y, float range, boolean air, boolean ground) {
				return Units.bestTarget(unit.team, x, y, range, u -> unit.checkTarget(air, ground), t -> ground, HUnitSorts.denser);
			}
			...
			public void draw(Unit unit, WeaponMount mount) {
				...
				Lines.stroke(4f * Mathf.clamp(mount.warmup / mount.weapon.minWarmup), HPal.matrixNet);
				...
				for (int i = 0; i < 3; i++) {
					Draws.drawTransform(mount.aimX, mount.aimY, 54, 0, -1.4f * Time.time + i * 120, (rx, ry, r) -> {
						Draw.rect(HIcon.matrixArrow.getRegion(), rx, ry, 12 * stLerp, 12 * stLerp, r + 90);
					});
				}
