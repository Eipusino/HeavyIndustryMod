...
public class EndDespondencyWeapon extends Weapon {
	public Seq<DespondencyArm> arms = new Seq<>(DespondencyArm.class);
	public TextureRegion[][] armRegions;
	...
	public void load() {
		...
		for (int i = 0; i < 2; i++) {
			...
			armRegions[i][0] = Core.atlas.find(MOD_NAME + "-despondency-h-arm" + outline);
			armRegions[i][1] = Core.atlas.find(MOD_NAME + "-despondency-f-arm" + outline);
			armRegions[i][2] = Core.atlas.find(MOD_NAME + "-despondency-hand" + outline);
			armRegions[i][3] = Core.atlas.find(MOD_NAME + "-despondency-hand2" + outline);
		}

		for (DespondencyArm arm : arms) {
			arm.weapon = Core.atlas.find(MOD_NAME + "-despondency-" + arm.name);
		}
	}
	...
	public void destroyStrays(Unit unit, DespondencyMount mount) {
		DesShockWaveEntity.create(unit.team, mount.target, unit.x, unit.y);
	}

	public void destroyStrong(Unit unit, DespondencyMount mount) {
	...
	public void destroyMain(Unit unit, DespondencyMount mount) {
		//
	}
	...
	protected static final Color[] lcolors = {HPal.red, HPal.redLight, Color.white, Color.black};
	...
	public static void drawLasers(int id, float x, float y, float rotation, float fout) {
		...
		for (int i = 0; i < 75; i++) {
			...
			Drawn.diamond(p1.x, p1.y, rwid, rheight, rotation);
		}
	}
	...
	public void drawMagic(Unit unit, DespondencyMount m) {
		...
		HEntity.progressiveCircle(unit.x, unit.y, rad1, 90f, fin);
		HEntity.progressiveCircle(unit.x, unit.y, rad1 / 3f, 180f, fin);
		HEntity.progressiveStar(unit.x, unit.y, rad1 / 3f, Time.time * 0.15f, 7, 2, fin);
		Lines.stroke(fout);
		HEntity.progressiveCircle(unit.x, unit.y, rad1 - 5f, -90f, fin);
		HEntity.progressiveCircle(unit.x, unit.y, rad1 / 2f, -90f, fin);
		HEntity.progressiveStar(unit.x, unit.y, rad1 / 2f, Time.time * 0.15f, 9, 2, fin);
		HEntity.progressiveStar(unit.x, unit.y, rad1 / 2f, Time.time * -0.15f, 4, 1, fin);
		Lines.stroke(3f * fout);
		HEntity.progressiveStar(unit.x, unit.y, rad1, Time.time * 0.1f, 4, 1, fin);
		HEntity.progressiveStar(unit.x, unit.y, rad1, -Time.time * 0.1f * 1.1f, 4, 1, fin);

		HEntity.progressiveStar(unit.x, unit.y, rad1, -90f, 3, 1, fin);
		for (int i = 0; i < 4; i++) {
			//draw circles
			float ang = i * 90f + Time.time * 0.2f;
			Vec2 v = Tmp.v1.trns(ang, rad1).add(unit.x, unit.y);
			Lines.stroke(3f * fout);
			HEntity.progressiveCircle(v.x, v.y, 90f, ang, fin);
			HEntity.progressiveStar(v.x, v.y, 90f, -90f, 5, 2, fin);
			Lines.stroke(fout);
			HEntity.progressiveCircle(v.x, v.y, 82f, ang, fin);
			HEntity.progressiveStar(v.x, v.y, 82f, 0f, 4, 1, fin);
			Lines.stroke(fout * 0.75f);
			HEntity.progressiveStar(v.x, v.y, 90f, 90f, 3, 1, fin);
	...
	public void drawFinalSpears(Unit unit, SpearHolder holder, float fade, TextureRegion[] set) {
	...
	public static class DespondencyMount extends WeaponMount {
		...
		public boolean finalActive;
		public float finalTime;
		public Seq<SpearHolder> spears = new Seq<>(SpearHolder.class);
		public DesSpearEntity lastSpear;
		public float spearTime;
		public boolean spearFinal = false;

		public boolean spearCollided = false;
		public float spearCollidedTime;

		public Seq<FragmentHands> fragHands = new Seq<>(FragmentHands.class);
	...
	protected static Vec2 v1 = new Vec2(), v2 = new Vec2(), v3 = new Vec2(), v4 = new Vec2(), v5 = new Vec2(), v6 = new Vec2();

	public class FragmentHands {
		public float time, timeB;
		public float fade;
		public float length;
		...
		public float hx, hy, jx, jy;

		public float cx, cy, cr;
		public FragmentEntity frag;
		public boolean holding, move2;
		...
		public void update(Unit unit, DespondencyMount mount) {
			...
			if (!holding) {
				...
				if (time >= 1f) {
					...
					if (frag.main.explosionSound != Sounds.none)
						frag.main.explosionSound.at(frag.x, frag.y, Mathf.random(0.9f, 1.1f) * Math.max(1f / (1f + (frag.area - 8f) / 70f), 0.5f), Mathf.clamp(frag.area / 8.1f));
	...
	public static class SpearHolder {
		public DesSpearEntity spear;
		public float x, y;
		public float fade;
		public float dst;
		public boolean flipped;

		public SpearHolder(DesSpearEntity spear, float x, float y, float dst) {
			this.spear = spear;
			this.x = x;
			this.y = y;
			this.dst = dst;
	...
	public class DespondencyArm {
		public float x, y, rotation = -30f;
		public float tx, ty, trot;
		public float length1 = 64f - 5f, length2 = 68f - 5f;
		public float insize = 5f;
		public float scl = 1f;
		public float tscl = 0f;

		public float armrot, handrot = 4f;
		public float delay;
		public float speed = 0.02f;

		public String name;
		public TextureRegion weapon;
		public float weaponOffset = 10f;
		public float weaponScl = 1f;

		public int id;

		public DespondencyArm(String name) {
			this.name = name;
		}

		public void draw(Unit unit, float progress, boolean flip) {
			int s = flip ? -1 : 1;
