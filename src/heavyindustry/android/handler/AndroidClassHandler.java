package heavyindustry.android.handler;

import dynamilize.*;
import dynamilize.classmaker.*;
import dynamilize.classmaker.code.*;
import heavyindustry.android.classes.*;
...
public class AndroidClassHandler implements ClassHandler {
    ...
    public AndroidClassHandler(ModInfo modInfo) {
        mod = modInfo;
        generatedLoader = new AndroidGeneratedClassLoader(modInfo, Vars.mods.mainLoader());
    }
                    ...
                    for (Method method : DynamicObject.class.getMethods()) {
                        ...
                        try {
                            implMethod = interType.getMethod(
                                    ...
                                    args.stream().map(ClassInfo::asType).toArray(Classc[]::new)
                            );
                        ...
                        CodeBlock<?> code = classInfo.declareMethod(
                                ...
                                method.getName(),
                                retType,
                                Parameterf.trans(Arrays.stream(method.getParameterTypes()).map(ClassInfo::asType).toArray(Classc[]::new))
                        );

                        if (implMethod.returnType() == ClassInfo.VOID_TYPE) {
                            code.invoke(null, implMethod, null, code.getParamAll().toArray(new Localc[0]));
                        } else {
                            Localc ret = code.local(retType);
                            code.invoke(null, implMethod, ret, code.getParamAll().toArray(new Localc[0]));
