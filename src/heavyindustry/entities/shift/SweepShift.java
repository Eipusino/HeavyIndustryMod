...
public class SweepShift extends ShiftHandler {
	...
	public SweepShift(String name) {
		...
		panels.addAll(new ShiftPanel() {{
			...
			heightCurve = a -> a < 0.7f ? Interp.pow2Out.apply(a) : Interp.pow2Out.apply(0.7f) * (1 - (a - 0.7f) / 0.3f);
			...
			//widthCurve = heightCurve = Interp.pow3Out;
		}}, new ShiftPanel() {{
			idx = 1;
			mirror = false;
	...
	public void drawIn(ApathyIUnit u, float progress) {
		...
		float offset = 0.07f;
		float s = Interp.pow2In.apply(Mathf.curve(progress, 0f, offset)) * 2f;
		if (progress > offset) {
			float f = (progress - offset) / (1f - offset);
			f = Mathf.curve(f, 0.1f, 0.25f);
			s = Mathf.lerp(2f, 1f, Interp.pow2In.apply(f));
		}
	...
	public void drawOut(ApathyIUnit u, float progress) {
		float af = Interp.pow2In.apply(Mathf.curve(progress, 0.2f, 0.75f));
		...
		TextureRegion r = regions[0];
		float fin = 1f - Interp.pow2Out.apply(progress);
