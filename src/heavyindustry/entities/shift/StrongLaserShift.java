...
public class StrongLaserShift extends ShiftHandler {
	public StrongLaserShift(String name) {
		...
		shiftSound = HSounds.largeTransform;

		critPoints = new float[]{
				0f, 20f
		};

		panels.addAll(new StrongLaserPanel() {{
			//x1 = x2 = -30f;
			x1 = -30f;
			x2 = -30f;
			y1 = 0f;
			...
			widthCurve = Interp.pow2In;
			heightCurve = Interp.pow3In;

			//moveCurve = rotCurve = a -> pow2.apply(a * a);
			moveCurve = rotCurve = Interp.pow2;
			...
			out = new StrongLaserPanel(this) {{
				widthCurve = heightCurve = Interp.pow3In;
			...
			moveCurve = rotCurve = a -> Interp.pow2.apply(Interp.pow2Out.apply(a));
