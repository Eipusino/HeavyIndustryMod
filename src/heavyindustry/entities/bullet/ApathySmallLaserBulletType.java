...
public class ApathySmallLaserBulletType extends BulletType {
	protected static final Seq<Healthc> tseq = new Seq<>(Healthc.class);

	public static float inEnd = 8f;

	public float length = 1900f;
	public float width = 8f;

	public Color[] colors = {HPal.primary.cpy().a(0.5f), HPal.primary, HPal.primary.cpy().mul(1.5f), Color.white};
	public float[] widths = {1.65f, 1f, 0.66f, 0.5f};
	...
	void handleData(ApathyData d, Healthc target) {
		...
		if (d.target == null || (d.target.dead() || !d.target.isAdded() || d.resetTimer <= 0f)) {
			if (d.target != null && (d.target.dead() || !d.target.isAdded())) {
				...
				if (!(d.target instanceof Building bl && target instanceof Building tb && (bl.tile == tb.tile)))
					d.ai.strongLaserScore = 0f;
	...
	public void update(Bullet b) {
		if (b.timer(0, 5f)) {
			...
			float vx = v.x;
			float vy = v.y;
			b.fdata = HDamage.hitLaser(b.team, width, b.x, b.y, vx, vy, null, t -> t.health() > damage * 20f, (t, x, y) -> {
				hit(b, x, y);
	...
	public void draw(Bullet b) {
		...
		//Drawn.drawShockWave(b.x, b.y, 75f, 0f, -b.rotation() - 90f, 200f, 4f, 12);
