...
/**
 * @deprecated see {@link HealConeBulletType}.
 * @since 1.0.6
 */
@Deprecated
public class HealingConeBulletType extends BulletType {
	...
	public HealingConeBulletType(float dmg) {
		damage = dmg;
		speed = 0.001f;
		hitEffect = Fx.none;
	...
	public void update(Bullet b) {
		...
		idx = 0;
		HDamage.shotgunRange(scanAccuracy, cone, b.rotation(), ang -> {
		...
		if (b.timer(1, 30f)) {
			...
			HDamage.shotgunRange(3, cone, b.rotation(), ang -> {
				Tmp.v1.trns(ang, length).add(b);
				Tmp.r1.merge(Tmp.v1);
			});
			...
			Groups.unit.intersect(Tmp.r1.x, Tmp.r1.y, Tmp.r1.width, Tmp.r1.height, unit -> {
				if (b.within(unit, length + (unit.hitSize / 2f)) && Angles.within(b.rotation(), b.angleTo(unit), cone)) {
					int index = Mathf.clamp(Mathf.round(((Mathm.angleDistSigned(b.angleTo(unit), b.rotation()) + cone) / (cone * 2f)) * (data.length - 1)), 0, data.length - 1);
			...
			HDamage.castConeTile(b.x, b.y, length, b.rotation(), cone, (building, tile) -> {
				if (building != null) {
