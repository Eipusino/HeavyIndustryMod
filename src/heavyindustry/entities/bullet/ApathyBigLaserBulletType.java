...
public class ApathyBigLaserBulletType extends BulletType {
	...
	Color[] colors = {HPal.primary.cpy().a(0.4f), HPal.primary, Color.white};
	...
	void handleDamage(Healthc e, Bullet b) {
		...
		if (e instanceof Unit u) {
			...
			if (b.time < 140f) {
				...
				u.vel.scl(acs);*/
				//HVars.listener.addMovementLock(u, 8f);
			} else {
				//HVars.listener.cancelMovementLock(u);
				u.impulse(Tmp.v3);
		...
		if (e instanceof Building bl) {
			...
			if (bl.team == Team.derelict && b.time >= 140f) {
				...
				HFx.aoeExplosion2.at(bl.x, bl.y, bl.hitSize() / 2f);
				bl.tile.remove();
			}
		}
		...
		if (b.time >= 140f) {
			...
			//HVars.listener.lockHealing(e, health - dam / 10f, 5f * 60f);
			e.damagePierce(dam);
		}
	...
	public void init(Bullet b) {
		...
		HFx.shootShockWave.at(Tmp.v1.x, Tmp.v1.y, b.rotation(), 120f);
	...
	public void update(Bullet b) {
		if (b.timer(0, 5f)) {
			...
			for (TeamData data : Vars.state.teams.present) {
				if (data.team != b.team) {
					if (data.unitTree != null) {
						HEntity.intersectLine(data.unitTree, dw, b.x, b.y, vx, vy, (u, x, y) -> {
							...
							if (u.dst(nearest) <= lw + u.hitSize / 2f) {
								hit(b, x, y);
								if (b.time > 140f) HFx.bigLaserHit.at(x, y, b.rotation(), u);
								handleDamage(u, b);
							}
						});
					}
					if (data.buildingTree != null) {
						HEntity.intersectLine(data.buildingTree, dw, b.x, b.y, vx, vy, (bl, x, y) -> {
							...
							if (bl.dst(nearest) <= lw + bl.hitSize() / 2f) {
								hit(b, x, y);
								if (b.time > 140f) HFx.bigLaserHit.at(x, y, b.rotation(), bl.hitSize());
								handleDamage(bl, b);
							}
						});
					}
			...
			HEntity.intersectLine((QuadTree<Bullet>) Groups.bullet.tree(), dw, b.x, b.y, vx, vy, (bl, x, y) -> {
		...
		if (b.fdata < 1 && b.time > 140f) {
			...
			HFx.shootShockWave.at(Tmp.v1.x, Tmp.v1.y, b.rotation(), 200f);
			Tmp.v1.trns(b.rotation(), 180f).add(b.x, b.y);
			HFx.shootShockWave.at(Tmp.v1.x, Tmp.v1.y, b.rotation(), 350f);

			//HVars.listener.impactFrames(b, b.x, b.y, b.rotation(), 23f, true);
	...
	public void draw(Bullet b) {
		...
		if (b.time > 140f) {
			...
			for (int i = 0; i < 75; i++) {
				...
				Drawn.diamond(v.x, v.y, dw, dh, b.rotation());
			}
		}
