...
public class ChainLightningBulletType extends BulletType {
	...
	public void init(Bullet b) {
		...
		for (int i = 0; i < Math.min(chainLightning, units.size); i++) {
			...
			HIDamage.list.add(unit);
			charges--;
		}

		HIDamage.list.each(u -> {
			HIDamage.chain(new Vec2(b.x, b.y), u, new IntSeq(), hitSound, hitEffect, b.damage, b.damage, width, distanceDamageFalloff, jumpDamageFactor, branches, segmentLength, arc, lightningColor);
		});
		...
		for (int i = 0; i < builds; i++) {
			...
			HIFx.chainLightning.at(Tmp.v1.x, Tmp.v1.y, 0, lightningColor, new LightningHolder(b, Tmp.v1));
	...
	public class LightningHolder implements HIFx.VisualLightningHolder {
