...
public class ModGetter {
	...
	/**
	 * Pass in a file and check if it is a mod file. If it is, return the mod's meta file. If not,
	 * throw {@link IllegalHandleException}.
	 *
	 * @param modFile Check the file, which can be a directory
	 * @return The main meta file of this mod
	 * @throws IllegalHandleException If this file is not a mod
	 */
	public static Fi checkModFormat(Fi modFile) throws IllegalHandleException {
		try {
			if (!(modFile instanceof ZipFi) && !modFile.isDirectory()) modFile = new ZipFi(modFile);
		} catch (Throwable e) {
			throw new IllegalHandleException("file was not a valid zipped file");
		}
		...
		if (!meta.exists()) throw new IllegalHandleException("mod format error: mod meta was not found");
