...
public class DPCliff extends Block {
	...
	public static void processCliffs() {
		world.tiles.eachTile(tile -> {
			if (tile.block() instanceof DPCliff && tile.data == 0) {
				for (int i = 0; i < 4; i++) {
					if (tile.nearby(i).block() instanceof DPCliffHelper) tile.data = (byte) (i + 1);
				}
				if (tile.data == 0) for (int i = 0; i < 4; i++) {
					if (tile.nearby(Geometry.d8edge(i)).block() instanceof DPCliffHelper) tile.data = (byte) (i + 5);
				}
				for (int i = 0; i < 4; i++) {
					if (tile.nearby(i).block() instanceof DPCliffHelper && tile.nearby((i + 1) % 4).block() instanceof DPCliffHelper) tile.data = (byte) (i + 9);
				}
				if (tile.data == 0) tile.setBlock(Blocks.air);
			}
		});
		world.tiles.eachTile(tile -> {
			if (tile.block() instanceof DPCliffHelper) Call.setTile(tile, Blocks.air, Team.derelict, 0);
		});
	}

	public static void unProcessCliffs() {
		world.tiles.eachTile(tile -> {
			if (HBlocks.cliffHelper != null && tile.block() instanceof DPCliff && tile.data != 0) {
				if (tile.data <= 4) {
