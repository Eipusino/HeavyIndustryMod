...
public class NextWave extends Block {
	public Cons2<Building, Table> buildConfig = (build, table) -> {
		table.button(Icon.upOpen, Styles.cleari, () -> build.configure(0)).size(50f).tooltip(Core.bundle.get("hi-next-wave-1"));
		table.button(Icon.warningSmall, Styles.cleari, () -> build.configure(1)).size(50f).tooltip(Core.bundle.get("hi-next-wave-10"));
	};
