...
import static heavyindustry.graphics.Draw3d.*;
...
public class ShieldProjector extends ForceProjector {
    ...
    public Effect shrinkEffect = Fx.none;
    ...
    public void setBars() {
        super.setBars();
        removeBar("shield");
        addBar("shield", (ShieldBuild tile) -> new Bar(
                () -> Core.bundle.get("stat.shieldhealth"),
                () -> tile.team.color,
                () -> tile.broken ? 0f : tile.shieldf()
        ).blink(Color.white));

        addBar("charge", (ShieldBuild tile) -> new Bar(
                () -> Core.bundle.format("bar.hi-charge", Strings.autoFixed(tile.charge / chargeTime * 100f, 2)),
                () -> tile.team.color,
                () -> tile.charge / chargeTime
        ));
    }

    @Override
    public void init() {
        clipSize = Math.max(clipSize, Mathm.cornerDst(radius + phaseRadiusBoost + 3f) * 2f);

        super.init();

        if (chargeEffect == Fx.none)
            chargeEffect = new Effect(20f, e -> {
        ...
        if (breakEffect == Fx.none)
            breakEffect = new Effect(40f, e -> {
                if (!(e.data instanceof ShieldSizeData data)) return;
                Lines.stroke(3f * e.fout(), e.color);
                float[] corners = getShieldCorners(e.x, e.y, e.rotation, data.sides, data.rotation);
                drawCorners(corners, data.height, data.sides, (x1, y1, x2, y2, x3, y3, x4, y4) -> {
                    Lines.line(x1, y1, x2, y2);
                    Lines.line(x1, y1, x4, y4);
                    Lines.line(x2, y2, x3, y3);
                    Lines.line(x4, y4, x3, y3);
        ...
        if (shrinkEffect == Fx.none)
            shrinkEffect = new Effect(20f, e -> {
                if (!(e.data instanceof ShieldSizeData data)) return;
                float radius = e.rotation * e.fout();
                if (radius > 0.001f) {
                    Draw.color(e.color);
                    float[] corners = getShieldCorners(e.x, e.y, radius, data.sides, data.rotation);
                    ...
                    Fill.polyBegin();
                    drawCorners(corners, data.height * e.fout(), data.sides, (x1, y1, x2, y2, x3, y3, x4, y4) -> {
    ...
    public class ShieldBuild extends ForceBuild {
        ...
        public void onRemoved() {
            ...
            if (!broken && radius > 1f)
                shrinkEffect.at(x, y, realRadius(), team.color, new ShieldSizeData(sides, shieldRotation, realHeight()));
        }
            ...
            if (buildup > 0) {
                ...
                if (!broken) {
                    ...
                    if (charge >= chargeTime) {
                        chargeEffect.at(x, y, 0f, team.color, this);
