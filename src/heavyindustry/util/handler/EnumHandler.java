...
/**
 * The enumeration processor provides some operation methods for enum, which can create enumeration instances and put them into the values of enumeration.
 * <p>This processor is an instance factory, and you need to construct an enumeration processor instance on the target enumeration class in order to perform operations.
 * <p>Since the property is a private final, reflection settings are required. Here, {@link FieldHandler} is referenced to complete the reflection operation. After you obtain an instance of the enumeration processor, you can directly reference the methods provided by the processor.
 * <p><strong>Note that due to the mindustry source code's declaration of enumeration, the operated enumeration may contain an array that holds all instances of the enumeration, but this is only when values() is called during initialization.
 * <p>But this array is a copy, if you have added items to the enumeration, it is best to reassign that array as the changed value()</strong>
 */
...
public class EnumHandler<T extends Enum<?>> {
    ...
    /**
     * Construct an enumeration processor without a constructor implementation using the target enumeration type.
     * If the enumeration has a constructor implementation, you must provide an external implementation of the constructor, otherwise unexpected errors may occur.
     *
     * @param clazz The target enumeration types processed by the processor
     */
     ...
     * If you don't put it in the sub item list of the enumeration, its ordinal can be arbitrarily specified, although this may cause unnecessary errors.
     *
     ...
     * The enumeration instances in the original position and the instances behind them will be moved backwards,
     * and the enumeration ordinals will be updated synchronously with the updates of the list.
     *
     * @param addition The name of the instance created
    ...
    /**
     * Place the specified enumeration item in the specified ordinal position, and reset all enumeration item ordinals to their correct positions.
     * The enumeration ordinal must be correctly specified between 0 and the total number of enumeration items,
     * and the ordinal number of newly added items can be equal to the current total number.
     *
     ...
     * @throws IndexOutOfBoundsException If the specified ordinal is greater than or equal to the current total number of elements in the enumeration, or if the ordinal of the newly added item is greater than the total number of elements
     */
