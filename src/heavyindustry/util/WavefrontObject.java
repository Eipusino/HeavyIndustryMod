...
public class WavefrontObject {
	...
	public void load(Fi file, @Nullable Fi material) {
		if (material != null) {
			try (BufferedReader matReader = material.reader(64)) {
		...
		try (BufferedReader reader = file.reader(64)) {
			Material current = null;
			while (true) {
	...
	public class Face {
		...
		protected void draw() {
			...
			for (int f = 0; f < (mat != null && mat.emitTex != null ? 2 : 1); f++) {
				...
				Draw.vert((atlas == null || !hasTexture) ? region.texture : atlas.texture, data, 0, data.length);
			}
		}
	}
