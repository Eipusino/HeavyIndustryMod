...
public class Fragmentation {
	...
	public Cons<? super FragmentEntity> onDeath;
	public Effect trailEffect = Fx2.debrisSmoke, explosionEffect = Fx2.fragmentExplosion;
	...
	public static Fragmentation generate(float x, float y, float rotation, float layer, float elevation, TextureRegion texture, Cons<? super FragmentEntity> cons) {
		return generate(x, y, rotation, texture.width * Draw.scl, texture.height * Draw.scl, layer, elevation, 1f, 1f, texture, cons);
	}

	public static Fragmentation generate(float x, float y, float rotation, float drawWidth, float drawHeight, float layer, float elevation, float resScale, float islandScl, TextureRegion texture, Cons<? super FragmentEntity> cons) {
		//this(region, Math.max(3, (int)((20f / 320) * region.width)), Math.max(3, (int)((20f / 320) * region.height)), Math.max(2, Math.min((int)((6f / (320f * 320f)) * region.width * region.height), 30)));
		...
		public void update() {
			...
			if (z <= 0f) {
				...
				if (!impact) {
					...
					Fx2.fragmentGroundImpact.at(x, y, boundSize / 2, Tmp.c1);
