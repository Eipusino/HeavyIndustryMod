...
public class ElectricStormBulletType extends BulletType {
	...
	public void update(Bullet b) {
		if (b.time >= b.lifetime - Fx2.chainLightningFade.lifetime) return;
		...
		if (b.timer.get(lifetime / 15f)) {
			...
			for (int i = 0; i < t.size; i++) {
				Healthc hc = t.get(i);
				if (hc != null) {
					Fx2.chainLightningFade.at(b.x, b.y, 4, color.cpy().a(0.3f), hc);
	...
	public void draw(Bullet b) {
		...
		if (b.timer.get(1, 27 * b.foutpow() + 3)) {
			...
			for (int i = 0; i < 3; i++) {
				...
				Fx2.chainLightningFade.at(x, y, 8, color, b);
			}
		}
	}
}
