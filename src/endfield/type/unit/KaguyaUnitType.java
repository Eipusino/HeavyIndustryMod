...
import static endfield.content.UnitTypes2.SHOOTERS;
import static endfield.content.UnitTypes2.TIMER;
	...
	public KaguyaUnitType(String name) {
		...
		Floatt2<Weapon> laser = (dx, dy) -> new Weapon(this.name + "-laser") {{
			...
			bullet = new LaserBulletType(165f) {{
				...
				colors = new Color[]{Pal2.matrixNetDark, Pal2.matrixNet, Color.white};
				hitColor = colors[0];
			}};
		}};
		...
		weapons.add(new Weapon(this.name + "-cannon") {{
			...
			bullet = new MultiTrailBulletType() {{
				...
				hitEffect = new MultiEffect(Fx.shockwave, Fx.bigShockwave, Fx2.impactWaveSmall, Fx2.spreadSparkLarge, Fx2.diamondSparkLarge);
				despawnHit = true;
				smokeEffect = Fx.shootSmokeSmite;
				shootEffect = Fx2.railShootRecoil;
			...
			public final BulletType subBull = new BulletType() {{
				...
				trailWidth = 1;
				trailLength = 38;
				hitEffect = Fx2.neutronWeaveMicro;
				despawnEffect = Fx2.constructSpark;
			}
				@Override
				public void draw(Bullet b) {
					super.draw(b);
			...
			public void update(Unit unit, DataWeaponMount mount) {
				...
				if (mount.warmup > 0.8f && timer.get(120)) {
					for (int i = 0; i < shooters.length; i++) {
						...
						Time.run(i * 40, () -> {
							Fx2.explodeImpWaveMini.at(unit.x + shooter.x, unit.y + shooter.y, Pal2.matrixNet);
							for (int l = 0; l < 10; l++) {
								Time.run(l * 4, () -> {
									Vec2 v = Mathm.fourierSeries(Time.time, shooter.param).scl(mount.warmup);
			...
			protected void shoot(Unit unit, DataWeaponMount mount, float shootX, float shootY, float rotation) {
				...
				Fx2.shootRecoilWave.at(shootX, shootY, rotation, Pal2.matrixNet);
				Fx2.impactWave.at(shootX, shootY, Pal2.matrixNet);

				Fx2.impactWave.at(mountX, mountY, Pal2.matrixNet);
				Fx2.crossLight.at(mountX, mountY, Pal2.matrixNet);
				Shooter[] shooters = mount.getVar(SHOOTERS);

				for (Shooter shooter : shooters) {
					Fx2.impactWaveSmall.at(mountX + shooter.x, mountY + shooter.y);
				}
			}
			...
			public void draw(Unit unit, DataWeaponMount mount) {
				...
				for (Shooter shooter : shooters) {
					Draw.color(Pal2.matrixNet);
					shooter.trail.draw(Pal2.matrixNet, 3 * mount.warmup);
					...
					Lines.stroke(3 * lerp, Pal2.matrixNet);
