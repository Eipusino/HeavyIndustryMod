...
import static endfield.content.UnitTypes2.EPHEMERAS;
		...
		weapons.addAll(new Weapon(this.name + "-cannon") {{
			...
			bullet = new MultiTrailBulletType() {{
				...
				despawnEffect = new MultiEffect(
						Fx2.impactWaveSmall,
						Fx2.spreadSparkLarge,
						Fx2.diamondSparkLarge
				);
			...
			bullet = new ContinuousBulletType() {{
				...
				hitEffect = Fx2.railShootRecoil;

				trailColor = hitColor = Pal2.matrixNet;
			...
			public void draw(Unit unit, DataWeaponMount mount) {
				...
				Draws.drawTransform(unit.x, unit.y, mount.weapon.x, mount.weapon.y, unit.rotation - 90, (x, y, r) -> {
					...
					for (Ephemera ephemera : list) {
						...
						ephemera.trail.draw(Pal2.matrixNet, 4f * ephemera.alpha);
					}
				});
			}
