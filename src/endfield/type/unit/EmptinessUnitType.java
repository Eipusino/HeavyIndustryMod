...
import endfield.entities.bullet.LightLaserBulletType;
...
import endfield.type.lightnings.TrailMoveLightning;
import endfield.type.lightnings.generator.RandomGenerator;
import endfield.type.lightnings.generator.VectorLightningGenerator;
...
import static endfield.content.UnitTypes2.PHASE;
import static endfield.content.UnitTypes2.STATUS;

public class EmptinessUnitType extends UnitType2 {
	private static final Rand rand = new Rand();

	public EmptinessUnitType(String name) {
		super(name);
		...
		class MayflyWeapon extends DataWeapon {
			...
			private MayflyWeapon() {
				super(EmptinessUnitType.this.name + "-mayfly");
				...
				bullet = new BulletType() {{
					...
					shootEffect = new MultiEffect(
							Fx2.impactBubbleSmall,
							Fx.colorSparkBig
					);
					hitColor = trailColor = Pal2.matrixNet;
					hitEffect = Fx2.diamondSparkLarge;
					trailEffect = new MultiEffect(Fx2.movingCrystalFrag);
					trailChance = 0.4f;
					despawnHit = true;
					speed = 10;
					lifetime = 120;
					...
					public void draw(Bullet b) {
						...
						Draws.drawDiamond(b.x, b.y, 24, 8 * delay, b.rotation());

						Draws.drawTransform(b.x, b.y, 4 * delay, 0, b.rotation(), (x, y, r) -> {
							Draws.gapTri(x, y, 12 * delay, 16 + 16 * delay, 14, r);
						});
					...
					public void updateHoming(Bullet b) {
						...
						if (b.time >= homingDelay) {
							...
							if (b.vel.len() >= speed * 0.8f) {
								if (b.timer(3, 3)) Fx2.weaveTrail.at(b.x, b.y, b.rotation(), hitColor);
							}
						}
					}
				};
				...
				final float[] farg2 = new float[9];
			{
				...
				public void update(Unit unit, DataWeaponMount mount) {
					...
					if (Mathf.chanceDelta(0.03f*mount.warmup)) {
						...
						Tmp.v1.rnd(3);
						Fx2.moveParticle.at(x + Tmp.v1.x, y + Tmp.v1.y, ang, Pal2.matrixNet, dst);
					}
					...
					tmp1.set(Mathm.fourierSeries(Time.time, farg1)).scl(mount.warmup);
					tmp2.set(Mathm.fourierSeries(Time.time, farg2)).scl(mount.warmup);
				...
				public void draw(Unit unit, DataWeaponMount mount) {
					...
					Draws.drawBloomUnderFlyUnit(null, ignored -> {
						trail.draw(Pal2.matrixNet, 4);
						Draw.color(Color.black);
						Fill.circle(x, y, 4);
						Draw.reset();
					});
					...
					trail1.draw(Pal2.matrixNet, 3*mount.warmup);
					trail2.draw(Pal2.matrixNet, 3*mount.warmup);

					Draw.color(Pal2.matrixNet);
					Fill.circle(x + tmp1.x, y + tmp1.y, 4f);
					Fill.circle(x + tmp2.x, y + tmp2.y, 4f);

					Draws.drawDiamond(x, y, 24, 10, angle);
					Draws.drawTransform(x, y, 0, 12, angle, (x, y, r) -> {
						Draws.gapTri(x, y, 12*mount.warmup, 22 + 24*mount.warmup, 8, r + 90);
					});
					Draws.drawTransform(x, y, 0, 10, angle - 180, (x, y, r) -> {
						Draws.gapTri(x, y, 9*mount.warmup, 12 + 8*mount.warmup, 6, r + 90);
					});
		...
		BulletType turretBullet = new BulletType() {{
			damage = 420;
			pierceCap = 4;
			pierceBuilding = true;
			laserAbsorb = true;
			...
			lifetime = 35;
			hitSize = 6;
			trailEffect = new MultiEffect(Fx2.trailLineLong, Fx.colorSparkBig);
			trailChance = 1;
			trailRotation = true;
		...
		weapons.addAll(new Weapon(this.name + "-turret") {{
			x = 17f;
			y = 26.5f;
			rotate = true;
			shootCone = 6;
			...
			bullet = new MultiTrailBulletType() {{
				...
				pierceCap = 5;
				pierceBuilding = true;
				hitEffect = new MultiEffect(Fx2.diamondSparkLarge, Fx2.spreadSparkLarge);
				despawnEffect = Fx2.explodeImpWaveSmall;
				hitShake = 6;
				hitSound = Sounds.shootArc;
				speed = 10;
				lifetime = 60;
				trailEffect = new MultiEffect(Fx.colorSparkBig, Fx2.movingCrystalFrag, Fx2.polyParticle);
				trailChance = 0.3f;
				trailColor = Pal2.matrixNet;
				trailRotation = true;
				shootEffect = Fx2.shootRail;
				...
				public void draw(Bullet b) {
					...
					Draw.color(hitColor);
					Draws.gapTri(b.x, b.y, 12, 28, -10, b.rotation());
				}
			};
		}}, new MayflyWeapon() {{
			x = 58.5f;
			...
			class BlastLaser extends LightningBulletType2 { {
				collides = false;
				hittable = false;
				absorbable = false;
				pierce = true;
				...
				public void createFrags(Bullet b, float x, float y) {
					if (fragBullet != null && fragBullets > 0) {
						Unit[] arr = UnitSorts2.findEnemies(fragBullets, b, fragBullet.range, (us, u) -> {
			...
			bullet = new BlastLaser() {{
				...
				intervalBullet = Bullets2.spilloverEnergy.copy();
				...
				laserEffect = new MultiEffect(Fx2.laserBlastWeaveLarge, Fx2.circleSparkLarge, Fx2.impactBubbleBig);
				shootEffect = new MultiEffect(Fx2.shootCrossLightLarge, Fx2.explodeImpWaveBig, Fx2.impactWaveBig, Fx2.impactBubble);
				hitEffect = new MultiEffect(Fx.colorSparkBig, Fx2.diamondSparkLarge);
				...
				fragBullet = new BlastLaser() {{
					...
					hitEffect = new MultiEffect(Fx.circleColorSpark, Fx2.diamondSparkLarge);

					blackZone = false;

					laserEffect = Fx2.explodeImpWaveLaserBlase;
					...
					fragBullets = 8;
					fragBullet = Bullets2.lightning(128, 32, 62, 5.2f, Pal2.matrixNet, b -> {
						generator.originAngle = b.rotation();
						return generator;
					});
			...
			parts.add(new CustomPart() {{
				...
				draw = (x, y, r, p) -> {
					...
					for (int i = 0; i < 4; i++) {
						...
						Draws.gapTri(rx, ry, Mathf.absin(Time.time / 4 - i * Mathf.pi, 1, (10 - 2 * i) * p), (10 + (6 + (i % 2) * 6) * p) - i, (i % 2 == 0 ? -1 : 1) * (5 + 4 * p - i), r);
					}

					Draws.drawDiamond(x, y, 44 + 20 * p, 8 + 4 * p, Time.time);
					Draws.drawDiamond(x, y, 38 + 14 * p, 6 + 4 * p, -Time.time * 1.2f);
					Draws.drawDiamond(x, y, 32 + 8 * p, 4 + 3 * p, Time.time * 1.3f);
					Fill.circle(x, y, 12f);
					Draw.color(Color.white);
					Fill.circle(x, y, 9f);
					Draw.color(Color.black);
			...
			protected Teamc findTarget(Unit unit, float x, float y, float range, boolean air, boolean ground) {
				return Units.bestTarget(unit.team, x, y, range, u -> unit.checkTarget(air, ground), t -> ground, UnitSorts2.denser);
			}
			...
			public void draw(Unit unit, WeaponMount mount) {
				...
				Draws.arc(mount.aimX, mount.aimY, 62, 360 * l, -Time.time * 1.2f);

				Lines.stroke(4f * Mathf.clamp(mount.warmup / mount.weapon.minWarmup), Pal2.matrixNet);
				Draws.drawCornerTri(mount.aimX, mount.aimY, 46, 8, Mathm.gradientRotateDeg(Time.time * 0.85f, 38, 1 / 3f, 3), true);

				Draw.z(Draw.z() + 0.01f);
				for (int i = 0; i < 3; i++) {
					Draws.drawTransform(mount.aimX, mount.aimY, 54, 0, -1.4f * Time.time + i * 120, (rx, ry, r) -> {
						Draw.rect(Icon2.matrixArrow.getRegion(), rx, ry, 12 * stLerp, 12 * stLerp, r + 90);
					});
				}
