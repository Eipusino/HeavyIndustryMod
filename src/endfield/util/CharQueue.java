...
public class CharQueue {
	...
	public void addLast(char object) {
		char[] theValues = values;

		if (size == theValues.length) {
			resize(theValues.length << 1);// * 2
			theValues = values;
		}

		theValues[tail++] = object;
		if (tail == theValues.length) {
			tail = 0;
		}
		size++;
	}
	...
	public void addFirst(char object) {
		...
		theHead--;
		if (theHead == -1) {
			theHead = theValues.length - 1;
		}
		theValues[theHead] = object;
