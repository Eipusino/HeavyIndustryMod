...
public class BombLauncher extends CommandableAttackerBlock {
	...
	public Sound shootSound = Sounds2.launch;
	...
	public void init() {
		super.init();
		if (bullet.shootEffect == Fx2.boolSelector)
			bullet.shootEffect = Fx2.square(baseColor, 50f, 6, size * Vars.tilesize * 2f, size);
	}
	...
	public class BombEntity extends CommandEntity implements Damagec {
		...
		public BombEntity(Team tea, float lif, Position fro, float xv, float yv, boolean par) {
			team = tea;
			lifetime = lif;
			parent = par;
			x = fro.getX();
			y = fro.getY();
			target = new Vec2(xv, yv);
			trail = new Trail(16);
		}

		public BombEntity init(Team tea, float lif, Position fro, float xv, float yv, boolean par) {
			team = tea;
			lifetime = lif;
			parent = par;
			x = fro.getX();
		...
		public BombEntity setDamage(float dam, float rad) {
			damage = dam;
			radius = rad;
			return this;
		}
