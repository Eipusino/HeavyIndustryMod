...
public class DepthCliff extends Block {
	...
	public static void unProcessCliffs() {
		world.tiles.eachTile(tile -> {
			if (Blocks2.cliffHelper != null && tile.block() instanceof DepthCliff && tile.data != 0) {
				if (tile.data <= 4) {
					tile.nearby(tile.data - 1).setBlock(Blocks2.cliffHelper);
				} else if (tile.data <= 8) {
					tile.nearby(Geometry.d8edge(tile.data - 5)).setBlock(Blocks2.cliffHelper);
				} else {
					tile.nearby(Geometry.d4(tile.data - 9)).setBlock(Blocks2.cliffHelper);
					tile.nearby(Geometry.d4(tile.data - 8)).setBlock(Blocks2.cliffHelper);
				}
				tile.data = 0;
			}
		});
