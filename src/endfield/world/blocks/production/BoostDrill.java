...
public class BoostDrill extends Drill {
	...
	public void setStats() {
		...
		stats.add(Stat.abilities, t -> {
			...
			t.table(Styles.grayPanel, nt -> {
				nt.add(Core.bundle.format("stat.double-get", upTimes, pop * 100)).pad(10).left();
				nt.table(p2 -> {
					p2.right().defaults().padRight(3).left();
					p2.add(Core.bundle.get("stat.double-get-can")).pad(8f).left();
					p2.row();
					p2.add(new ItemDisplay(boostItem, 1, consumeTime, true)).pad(10f).left();
					p2.add(Core.bundle.format("stat.double-get-boost", (popAfter - pop) * 100)).left();
	...
	public void init() {
		...
		blockedItem = boostItem;
		boostEffect = Fx2.diffuse(size, boostItem.color, 30);
	...
	public class BoostDrillBuild extends DrillBuild {
		...
		public MeshResult result = Icosphere.create(0);
		...
		public boolean acceptItem(Building source, Item item) {
			return item == boostItem && items.get(item) < getMaximumAccepted(item);
		}
		...
		public void draw() {
			...
			for (int i = 0; i < result.indices.size - 1; i += 3) {
				Vec3 v31 = Tmp.v31.set(result.vertices.items, result.indices.items[i] * 3).setLength(size * 5 * w);
				Vec3 v32 = Tmp.v32.set(result.vertices.items, result.indices.items[(i + 1)] * 3).setLength(size * 5 * w);
				Vec3 v33 = Tmp.v33.set(result.vertices.items, result.indices.items[(i + 2)] * 3).setLength(size * 5 * w);
