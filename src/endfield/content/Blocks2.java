...
import endfield.entities.part.DrawArrowSequence;
import endfield.entities.part.FlipRegionPart;
...
public final class Blocks2 {
	...
	public static void load() {
		...
		starfall = new ItemTurret("starfall") {{
			requirements(Category.turret, ItemStack.with(Items.graphite, 2500, Items.plastanium, 1200, Items.surgeAlloy, 1500, Items2.crystallineCircuit, 800, Items2.uranium, 1200));
			size = 8;
			sync = true;
			health = 42500;
			armor = 110f;
			lightColor = Pal2.ancientLightMid;
			clipSize = 6 * 24;
			outlineColor = Pal.darkOutline;
			ammo(Items2.chromium, new CtrlMissileBulletType(4.2f, 2800f, MOD_NAME + "-large-missile") {{
				hitSize *= 10f;
				accel = 0.32f;
				drag /= 2;
				lifetime = 120f;
				...
				status = StatusEffects2.ultFireBurn;
				splashDamage = 4200f;
				...
				fragBullets = 8;
				fragBullet = Bullets2.arc9000frag;
				ammoMultiplier = 1f;
				chargeEffect = Fx2.railShoot(Pal.techBlue, 800f, 18, Fx2.techBlueChargeBegin.lifetime, 25);
				shootEffect = Fx2.instShoot(Pal.techBlue, Pal.techBlue);
				...
				hitEffect = new MultiEffect(Fx2.largeTechBlueHit, Fx2.blast(Pal.techBlue, 140f), Fx2.largeTechBlueHitCircle, Fx2.subEffect(150, splashDamageRadius * 0.66f, 13, 34f, Interp.pow2Out, ((i, x, y, rot, fin) -> {
					float fout = Interp.pow2Out.apply(1 - fin);
					float finPow = Interp.pow3Out.apply(fin);
					Tmp.v1.trns(rot, 25 * finPow);
					for (int s : Mathf.signs) {
						Drawf.tri(x, y, 12 * fout, 45 * Mathf.curve(finPow, 0, 0.3f) * Fx2.fout(fin, 0.15f), rot + s * 90);
					}
				})));
			}});
			shoot = new ShootBarrel() {{
				...
				firstShotDelay = Fx2.techBlueChargeBegin.lifetime;
				shots = 3;
				shotDelay = 12f;
			}};
		...
		solstice = new PowerTurret("solstice") {{
			...
			clipSize = 8 * 24;
			outlineColor = Pal.darkOutline;
			requirements(Category.turret, ItemStack.with(Items.plastanium, 2000, Items.surgeAlloy, 2500, Items.phaseFabric, 1600, Items2.crystallineCircuit, 1100, Items2.galliumNitride, 1800, Items2.heavyAlloy, 2200));
			warmupMaintainTime = 90f;
			shootWarmupSpeed /= 5f;
			minWarmup = 0.9f;
			shootCone = 15f;
			rotateSpeed = 0.325f;
			canOverdrive = false;
			shootType = Bullets2.heavyArtilleryProjectile;
			ammoPerShot = 12;
			maxAmmo = 180;
			range = 1200;
			reload = 120;
		...
		annihilate = new PowerTurret("annihilate") {{
			...
			coolant = consume(new ConsumeLiquid(Liquids2.crystalFluid, 20f / 60f));
			coolantMultiplier = 1.5f;
			moveWhileCharging = false;
			canOverdrive = false;
			...
			shoot.firstShotDelay = Fx2.techBlueChargeBegin.lifetime;
			chargeSound = Sounds.none;
			requirements(Category.turret, ItemStack.with(Items.plastanium, 1500, Items.surgeAlloy, 1100, Items2.crystallineCircuit, 400, Items2.chromium, 800));
			shootType = Bullets2.arc9000;
			shootCone = 12f;
			rotateSpeed = 0.75f;
			ammoPerShot = 4;
			maxAmmo = 20;
		...
		executor = new PowerTurret("executor") {{
			requirements(Category.turret, ItemStack.with(Items.plastanium, 2200, Items.surgeAlloy, 3500, Items.phaseFabric, 4200, Items2.crystallineElectronicUnit, 1700, Items2.heavyAlloy, 4500));
			size = 16;
			health = 1000000;
			armor = 150;
			rotateSpeed = 0.75f;
			...
			inaccuracy = 1.5f;
			velocityRnd = 0.075f;
			shootSound = Sounds2.railGunBlast;
			chargeSound = Sounds.none;
			canOverdrive = false;
			warmupMaintainTime = 50f;
		...
		heatDeath = new PowerTurret("heat-death") {{
			...
			range = 1200;
			heatColor = Pal.techBlue;
			unitSort = UnitSorts2.regionalHPMaximumAll;
			coolant = consume(new ConsumeLiquid(Liquids2.crystalFluid, 1f));
			coolantMultiplier = 0.5f;
			liquidCapacity = 120;
			buildCostMultiplier = 0.8f;
			canOverdrive = false;
			...
			consumePower(800f);
			reload = 1800f;
			shootType = Bullets2.arc9000hyper;
			requirements(Category.turret, ItemStack.with(Items2.crystallineCircuit, 2200, Items2.crystallineElectronicUnit, 1100, Items2.heavyAlloy, 5500));
			buildType = PowerTurretBuild::new;
		}};
		//turrets-erekir
