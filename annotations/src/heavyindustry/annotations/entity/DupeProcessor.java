...
import heavyindustry.annotations.Annotations.Actually;
import heavyindustry.annotations.Annotations.BreakAll;
import heavyindustry.annotations.Annotations.Dupe;
import heavyindustry.annotations.Annotations.DupeComponent;
import heavyindustry.annotations.Annotations.DupeInterface;
import heavyindustry.annotations.Annotations.Ignore;
...
public class DupeProcessor extends BaseProcessor {
	...
	DupeDefinition toClass(Element def, ObjectMap<String, Element> usedNames, boolean isBuild) {
		...
		for (Entry<String, Seq<ExecutableElement>> entry : methods) {
			...
			mbuilder.addAnnotations(Seq.with(first.getAnnotationMirrors()).map(AnnotationSpec::get).removeAll(a -> a.type.equals(typeName(Actually.class))));
			...
			if (first.getReturnType().getKind() != TypeKind.VOID && !inserts.isEmpty()) {
	...
	boolean processDefinition(DupeDefinition def) {
		...
		if (def.parent != null) {
			def.builder.superclass(className(findBuild(block)));
			for (TypeElement extraInterface : Seq.with(findBuild(based).getInterfaces()).map(this::toElement).select(i -> unCompInterface(i))) {
				def.builder.addSuperinterface(className(extraInterface));
			}
			def.parent.builder.addType(def.builder.build());
		} else {
			def.builder.superclass(typeName(block));
			for (TypeElement extraInterface : Seq.with(based.getInterfaces()).map(this::toElement).select(i -> unCompInterface(i))) {
				def.builder.addSuperinterface(className(extraInterface));
			}
		}
	...
	boolean unCompInterface(TypeElement type) {
