...
import heavyindustry.annotations.Annotations.Combine;
...
import heavyindustry.annotations.Annotations.Resolve.Method;
...
import heavyindustry.annotations.util.TypeIOResolver.ClassSerializer;
...
public class EntityProcessor extends BaseProcessor {
	...
	public void process(RoundEnvironment roundEnv) throws Exception {
		...
		if (round == 1) {
			...
			for (TypeElement comp : comps) {
				...
				if (compAnno.write()) {
					TypeSpec.Builder inter = TypeSpec.interfaceBuilder(interfaceName(comp))
							...
							.addAnnotation(className(EntityInterface.class))
							.addAnnotation(
									AnnotationSpec.builder(className(SuppressWarnings.class))
											.addMember("value", "{$S, $S}", "all", "deprecation")
											.build()
							);

					for (TypeElement extraInterface : Seq.with(comp.getInterfaces()).map(this::toElement).select(i -> !isCompInterface(i))) {
						inter.addSuperinterface(className(extraInterface));
					}
					...
					for (VariableElement var : vars(comp).select(v -> !is(v, Modifier.STATIC) && !is(v, Modifier.PRIVATE) && annotation(v, Import.class) == null)) {
						...
						if (
							...
							inter.addMethod(
									MethodSpec.methodBuilder(name)
											.addModifiers(Modifier.PUBLIC, Modifier.ABSTRACT)
											.addParameter(typeName(var), name)
						...
						if (annotation(comp, EntityDef.class) == null) {
							...
							for (TypeElement dep : deps) {
								for (VariableElement var : vars(dep).select(v -> !is(v, Modifier.PRIVATE) && !is(v, Modifier.STATIC) && annotation(v, Import.class) == null && annotation(v, ReadOnly.class) == null)) {
									FieldSpec.Builder field = FieldSpec.builder(typeName(var), simpleName(var), Modifier.PUBLIC);
			...
			for (Element def : defs) {
				...
				for (TypeElement comp : defComps) {
					...
					for (VariableElement field : fields) {
						...
						FieldSpec.Builder fbuilder = FieldSpec.builder(typeName(field), simpleName(field));
				...
				if (!methods.containsKey("toString()")) {
					builder.addMethod(
							MethodSpec.methodBuilder("toString")
									.addAnnotation(className(Override.class))
				...
				for (Entry<String, Seq<ExecutableElement>> entry : methods) {
					...
					if (!isPrivate && !is(first, Modifier.STATIC)) mbuilder.addAnnotation(className(Override.class));
					...
					for (VariableElement var : first.getParameters()) {
						mbuilder.addParameter(typeName(var), simpleName(var));
					}
					...
					if (simpleName(first).equals("remove") && ann.pooled()) {
						mbuilder.addStatement("$T.queueFree(this)", className(Groups.class));
					}
			...
			TypeSpec.Builder map = TypeSpec.classBuilder("EntityRegister").addModifiers(Modifier.PUBLIC)
					.addField(
							FieldSpec.builder(ParameterizedTypeName.get(
											className(ObjectIntMap.class),
											ParameterizedTypeName.get(
													className(Class.class),
													WildcardTypeName.subtypeOf(className(Entityc.class))
											)
									), "ids")
									.addModifiers(Modifier.PRIVATE, Modifier.STATIC, Modifier.FINAL)
									.initializer("new $T<>()", className(ObjectIntMap.class))
									.build()
					)
					.addField(
					...
					.addMethod(
							MethodSpec.methodBuilder("register")
									...
									.returns(TypeName.VOID)
									.addTypeVariable(typeVarName("T", className(Entityc.class)))
									.addParameter(
											ParameterizedTypeName.get(className(Class.class), typeVarName("T")),
											"type"
									)
									.addParameter(
											ParameterizedTypeName.get(className(Prov.class), typeVarName("T")),
											"prov"
									)
									.beginControlFlow("synchronized($T.class)", ClassName.get(packageName, "EntityRegister"))
									.addStatement("if(ids.containsKey(type) || $T.nameMap.containsKey(type.getSimpleName())) return", className(EntityMapping.class))
									.addCode(lnew())
									.beginControlFlow("for(; last < $T.idMap.length; last++)", className(EntityMapping.class))
									.beginControlFlow("if($T.idMap[last] == null)", className(EntityMapping.class))
									.addStatement("$T.idMap[last] = prov", className(EntityMapping.class))
									.addStatement("ids.put(type, last)")
									.addCode(lnew())
									.addStatement("$T.nameMap.put(type.getSimpleName(), prov)", className(EntityMapping.class))
									.addStatement("$T.nameMap.put($T.camelToKebab(type.getSimpleName()), prov)", className(EntityMapping.class), className(Strings.class))
									.addCode(lnew())
									.addStatement("break")
									.endControlFlow()
									.endControlFlow()
					...
					.addMethod(
							MethodSpec.methodBuilder("register")
									...
									.returns(TypeName.VOID)
									.addTypeVariable(typeVarName("T", className(Entityc.class)))
									.addParameter(className(String.class), "name")
									.addParameter(
											ParameterizedTypeName.get(className(Class.class), typeVarName("T")),
											"type"
									)
									...
									.addStatement("$T.nameMap.put(name, prov)", className(EntityMapping.class))
									.addCode(lnew())
									.addStatement("int id = classId(type)")
									.beginControlFlow("if(id != -1)")
									.addStatement("$T.customIdMap.put(classId(type), name)", className(EntityMapping.class))
									.endControlFlow()
									.build()
					)
					.addMethod(
							MethodSpec.methodBuilder("register")
									.addModifiers(Modifier.PUBLIC, Modifier.STATIC)
									.returns(TypeName.VOID)
									.addTypeVariable(typeVarName("T", className(Unit.class)))
									.addParameter(className(UnitType.class), "unit")
									.addParameter(
											ParameterizedTypeName.get(className(Class.class), typeVarName("T")),
											"type"
									)
			...
			for (Element e : pointers) {
				...
				TypeElement type = toElement(isUnit ? elements(point::value).first().asType() : e.asType());
				...
				if (isUnit) {
					...
					init.addStatement("register($T.$L, $T.class, $T::$L)", TypeName.get(up), c, className(type), className(type), constructor);

					usedNames.add(simpleName(type));
				} else if (!usedNames.contains(simpleName(type))) {
					init.addStatement("register($T.class, $T::$L)", className(type), className(type), constructor);
			...
			for (EntityDefinition def : definitions) {
				...
				for (TypeElement comp : def.components) {
					...
					def.builder.addSuperinterface(className(inter));
	...
	boolean append(MethodSpec.Builder mbuilder, Seq<TypeElement> defComps, Seq<ExecutableElement> values, Seq<ExecutableElement> inserts, Seq<ExecutableElement> wrappers, boolean writeBlock) {
		...
		if (values.contains(m -> annotation(m, Combine.class) != null)) {
			...
			for (JCStatement stat : trees.getTree(base).getBody().getStatements()) {
				if (stat instanceof JCVariableDecl var) {
	...
	Seq<TypeElement> getDependencies(TypeElement component) {
		if (!componentDependencies.containsKey(component)) {
			...
			Seq<TypeElement> list = Seq.with(component.getInterfaces())
					.map(i -> toComp(compName(simpleName(toElement(i)))))
					.select(Objects::nonNull);
	...
	private record EntityDefinition(String name, TypeSpec.Builder builder, Element naming, TypeName extend,
	                                Seq<TypeElement> components, Seq<String> groups, Seq<FieldSpec> fieldSpecs) {
