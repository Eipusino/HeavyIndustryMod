...
import heavyindustry.annotations.Annotations.BreakAll;
import heavyindustry.annotations.Annotations.BypassGroupCheck;
import heavyindustry.annotations.Annotations.EntityBaseComponent;
import heavyindustry.annotations.Annotations.EntityComponent;
import heavyindustry.annotations.Annotations.EntityDef;
import heavyindustry.annotations.Annotations.EntityInterface;
import heavyindustry.annotations.Annotations.EntityPoint;
import heavyindustry.annotations.Annotations.ExcludeGroups;
import heavyindustry.annotations.Annotations.Extend;
import heavyindustry.annotations.Annotations.Import;
import heavyindustry.annotations.Annotations.Insert;
import heavyindustry.annotations.Annotations.InternalImpl;
import heavyindustry.annotations.Annotations.MethodPriority;
import heavyindustry.annotations.Annotations.ReadOnly;
import heavyindustry.annotations.Annotations.Remove;
import heavyindustry.annotations.Annotations.Replace;
import heavyindustry.annotations.Annotations.SyncField;
import heavyindustry.annotations.Annotations.Wrap;
import heavyindustry.annotations.BaseProcessor;
import heavyindustry.annotations.TypeIOResolver;
...
/**
 * Is the reason for retention that someone may need it?
 *
 * @deprecated We don't need this now.
 */
@Deprecated
public class EntityProcessor extends BaseProcessor {
	protected static final Seq<MethodSymbol> tmpMethods = new Seq<>(MethodSymbol.class);
	protected static final Seq<Object> tmpArgs = new Seq<>(Object.class);
	protected static final Comparator<MethodSymbol> methodSorter = Structs.comps(Structs.comparingFloat(m -> {
	...
	protected Seq<ClassSymbol> baseComps = new Seq<>(ClassSymbol.class);
	...
	protected Seq<Symbol> defs = new Seq<>(Symbol.class);
	...
	protected Seq<ClassSymbol> pointers = new Seq<>(ClassSymbol.class);
	...
	protected Seq<EntityDefinition> definitions = new Seq<>(EntityDefinition.class);
	...
	public synchronized void init(ProcessingEnvironment env) {
		...
		try {
			String dir = Fi.get(filer.getResource(StandardLocation.CLASS_OUTPUT, "no", "no")
	...
	protected void process() throws IOException {
		switch (round) {
			case 1 -> {
				...
				pointer:
				for (ClassSymbol t : this.<ClassSymbol>with(EntityPoint.class)) {
					for (Symbol s : t.getEnclosedElements()) {
						if (s.getKind() == ElementKind.METHOD) {
							MethodSymbol m = (MethodSymbol) s;
							if (is(m, Modifier.PUBLIC, Modifier.STATIC) && m.params.isEmpty() && same(m.getReturnType(), t)) {
								pointers.add(t);
								continue pointer;
							}
						}
				...
				for (MethodSymbol e : this.<MethodSymbol>with(Insert.class)) {
					...
					inserters.get(type, () -> {
						...
						return map;
					}).get(anno(e, Insert.class).value(), () -> new Seq<>(false, 16, MethodSymbol.class)).add(e);
				}
				...
				for (MethodSymbol e : this.<MethodSymbol>with(Wrap.class)) {
					...
					wrappers.get(type, () -> {
						...
						map.orderedKeys().ordered = false;
						return map;
					}).get(anno(e, Wrap.class).value(), () -> new Seq<>(false, 16, MethodSymbol.class)).add(e);
				}
				...
				for (Symbol s : elements.getPackageElement("mindustry.gen").getEnclosedElements()) {
					String name = name(s);
					if (name.endsWith("c") && s.getKind() == ElementKind.INTERFACE && comp(compName(name)) != null) {
				...
				for (ClassSymbol comp : comps.values()) {
					for (Symbol s : comp.getEnclosedElements()) {
						...
						} else if (s.getKind() == ElementKind.METHOD && s.getKind() != ElementKind.CONSTRUCTOR) {
							MethodSymbol m = (MethodSymbol) s;
							if (isAny(m, Modifier.ABSTRACT, Modifier.NATIVE)) continue;
					...
					if (!compAnno.vanilla()) {
						...
						for (Type ext : comp.getInterfaces())
						...
						for (Symbol s : comp.getEnclosedElements()) {
							if (isAny(s, Modifier.PRIVATE, Modifier.STATIC) || s.getKind() != ElementKind.METHOD) continue;
						...
						for (Symbol s : comp.getEnclosedElements()) {
							if (isAny(s, Modifier.PRIVATE, Modifier.STATIC) || s.getKind() != ElementKind.FIELD || anno(s, Import.class) != null)
								continue;
							...
							if (!signatures.contains(name + "()")) {
								...
								for (Attribute.Compound anno : v.getAnnotationMirrors()) {
							...
							if (!is(v, Modifier.FINAL) && anno(v, ReadOnly.class) == null && !signatures.contains(name + "(" + v.type + ")")) {
							...
							if (anno(comp, EntityDef.class) == null) {
								...
								for (ClassSymbol dep : baseDeps) {
									for (Symbol s : dep.getEnclosedElements()) {
										if (s.getKind() == ElementKind.FIELD && !isAny(s, Modifier.PRIVATE, Modifier.STATIC) && anno(s, Import.class) == null && anno(s, ReadOnly.class) == null) {
											VarSymbol v = (VarSymbol) s;
											FieldSpec.Builder field = FieldSpec.builder(spec(v.type), name(v), Modifier.PUBLIC);

											if (is(v, Modifier.TRANSIENT)) field.addModifiers(Modifier.TRANSIENT);
											if (is(v, Modifier.VOLATILE)) field.addModifiers(Modifier.VOLATILE);
											for (Attribute.Compound anno : v.getAnnotationMirrors()) field.addAnnotation(spec(anno));
			...
			case 2 -> {
				...
				Seq<String> defGroups = new Seq<>(false, 16, String.class);
				...
				Seq<VarSymbol> syncedFields = new Seq<>(VarSymbol.class);
				Seq<VarSymbol> allFields = new Seq<>(VarSymbol.class);
				Seq<FieldSpec> allFieldSpecs = new Seq<>(FieldSpec.class);

				ObjectMap<String, Seq<MethodSymbol>> allInserters = new ObjectMap<>();
				ObjectMap<String, Seq<MethodSymbol>> allWrappers = new ObjectMap<>();
				Seq<MethodSymbol> bypass = new Seq<>(MethodSymbol.class), inserts = new Seq<>(MethodSymbol.class), wraps = new Seq<>(MethodSymbol.class);
				...
				for (Symbol def : defs) {
					...
					for (ClassSymbol comp : defComps.values())
						for (ClassSymbol dep : dependencies(comp)) defCompsResolve.put(name(dep), dep);
					...
					for (ClassSymbol comp : defComps.values())
						if (!excludeGroups.contains(comp) && groups.containsKey(comp)) defGroups.add(groups.get(comp));
					...
					for (ClassSymbol comp : defComps.values()) {
						...
						if (tmp != null)
							for (String key : tmp.orderedKeys()) allInserters.get(key, Seq::new).addAll(tmp.get(key));

						tmp = wrappers.get(comp);
						if (tmp != null)
							for (String key : tmp.orderedKeys()) allWrappers.get(key, Seq::new).addAll(tmp.get(key));

						boolean isShadowed = baseClassType != null && !typeIsBase && baseDependencies.get(baseClassType).contains(comp);
						for (Symbol s : comp.getEnclosedElements()) {
							if (s.getKind() == ElementKind.FIELD && anno(s, Import.class) == null) {
								VarSymbol v = (VarSymbol) s;
								...
								if (is(v, Modifier.STATIC)) {
									field.addModifiers(Modifier.STATIC);
									if (is(v, Modifier.FINAL)) field.addModifiers(Modifier.FINAL);
								}

								if (is(v, Modifier.TRANSIENT)) field.addModifiers(Modifier.TRANSIENT);
								if (is(v, Modifier.VOLATILE)) field.addModifiers(Modifier.VOLATILE);
								if (is(v, Modifier.PRIVATE)) {
									field.addModifiers(Modifier.PRIVATE);
								} else if (anno(v, ReadOnly.class) != null) {
									field.addModifiers(Modifier.PROTECTED);
								} else {
									field.addModifiers(Modifier.PUBLIC);
								}
								...
								boolean isVisible = !isAny(v, Modifier.STATIC, Modifier.PRIVATE) && anno(v, ReadOnly.class) == null;
								...
								if (isSync && anno(v, SyncField.class) != null) {
									if (v.type.getKind() != TypeKind.FLOAT) {
										err("All @SyncFields must be float.", v);
										continue;
									}

									syncedFields.add(v);
									builder.addField(FieldSpec.builder(TypeName.FLOAT, fname + EntityIO.targetSuffix, Modifier.TRANSIENT, Modifier.PRIVATE).build());
									builder.addField(FieldSpec.builder(TypeName.FLOAT, fname + EntityIO.lastSuffix, Modifier.TRANSIENT, Modifier.PRIVATE).build());
								}
							} else if (s.getKind() == ElementKind.METHOD) {
								MethodSymbol m = (MethodSymbol) s;
					...
					for (var entry : methods.entries()) {
						...
						VarSymbol setter = allFields.find(v -> {
							...
							return
									name(v).equals(name(first)) &&
											...
											first.getReturnType().getKind() == TypeKind.VOID;
						});

						if (setter != null) {
							ClassSymbol c = setter.enclClass();
							String n = name(setter);
							for (MethodSymbol m : entries) {
								if (m.enclClass() == c && n.equals(name(m)) && m.getReturnType().getKind() == TypeKind.VOID && m.params.size() == 1 && same(m.params.get(0).type, setter.type)) {
									setter = null;
									break;
								}
							}
						...
						if (topReplacer != null && (topReplace = anno(topReplacer, Replace.class)) != null) {
							...
							if (topReplacer.getReturnType().getKind() == TypeKind.VOID) {
								entries.removeAll(m -> {
									Replace rep = anno(m, Replace.class);
						...
						if (entries.count(m -> !isAny(m, Modifier.ABSTRACT, Modifier.NATIVE) && m.getReturnType().getKind() != TypeKind.VOID) > 1) {
						...
						if (entries.size > 1 && mname.equals("serialize") && m.getReturnType().getKind() == TypeKind.BOOLEAN && m.params.isEmpty()) {
							serializeOverride = true;
						} else if (anno(m, InternalImpl.class) != null) {
							continue;
						}

						boolean isPrivate = is(m, Modifier.PRIVATE);
						...
						if (!isPrivate && !is(m, Modifier.STATIC)) methBuilder.addAnnotation(spec(Override.class));
						if (is(m, Modifier.STATIC)) methBuilder.addModifiers(Modifier.STATIC);
						...
						boolean writeBlock = m.getReturnType().getKind() == TypeKind.VOID && entries.size > 1;
						if (isAny(m, Modifier.ABSTRACT, Modifier.NATIVE) && entries.size == 1 && anno(m, InternalImpl.class) == null) {
						...
						for (MethodSymbol e : standaloneInserts)
					...
					if (defAnno.pooled()) {
						...
						for (FieldSpec spec : allFieldSpecs) {
							...
							if (v == null || isAny(v, Modifier.STATIC, Modifier.FINAL)) continue;
				...
				TypeSpec.Builder registry = TypeSpec.classBuilder("EntityRegistry")
						...
						.addMethod(
								MethodSpec.methodBuilder("get")
										...
										.addStatement("return ($T)needIdMap.get(name)", paramSpec(spec(Prov.class), tvSpec("T")))
										.build()
						)
						.addMethod(
								MethodSpec.methodBuilder("getId")
										...
										.addStatement("return classIdMap.get(type, -1)")
										.build()
						)
						.addMethod(
								MethodSpec.methodBuilder("register")
										...
										.addStatement("needIdMap.put(name, prov)")
										.addStatement("classIdMap.put(type, $T.register(name, prov))", spec(EntityMapping.class))
										.build()
						)
						.addMethod(
				...
				for (EntityDefinition def : definitions) {
					...
					ObjectSet<String> methodNames = def.components.flatMap(t -> {
						Seq<String> out = new Seq<>(String.class);
						for (Symbol s : t.getEnclosedElements())
							if (s.getKind() == ElementKind.METHOD) out.add(sigName((MethodSymbol) s));
					...
					for (ClassSymbol comp : def.components) {
						...
						for (Symbol s : inter.getEnclosedElements()) {
							if (s.getKind() == ElementKind.METHOD) {
								...
								String variable = name(m);
								FieldSpec field = def.fieldSpecs.find(f -> f.name.equals(variable));
								if (field == null || methodNames.contains(sigName(m))) continue;

								MethodSpec result = null;
								if (m.getReturnType().getKind() != TypeKind.VOID) {
									result = MethodSpec.methodBuilder(variable)
											.addModifiers(Modifier.PUBLIC)
											.addAnnotation(spec(Override.class))
											.returns(spec(m.getReturnType()))
											.addStatement("return $L", variable)
											.build();
									...
									result = MethodSpec.methodBuilder(variable)
											...
											.addAnnotation(spec(Override.class))
											.returns(TypeName.VOID)
											.addParameter(field.type, variable)
											.addStatement("this.$L = $L", variable, variable)
											.build();
								}

								if (result != null) {
									if (superclass != null) {
										FieldSpec superField = Seq.with(superclass.fieldSpecs).find(f -> f.name.equals(variable));
										if (superField != null) {
											TypeName ret = result.returnType;
											MethodSpec targetMethod = Seq.with(superclass.methodSpecs).find(e -> e.name.equals(variable) && e.returnType.equals(ret));
										...
										for (Symbol sym : ext.getEnclosedElements()) {
											if (sym.getKind() == ElementKind.FIELD && name(sym).equals(variable))
										...
										if (superField != null) {
											...
											for (Symbol sym : ext.getEnclosedElements()) {
												if (sym.getKind() == ElementKind.METHOD && name(sym).equals(variable)) {
				...
				for (TypeSpec.Builder base : baseClasses.values()) {
					imports.clear();
					for (ClassSymbol dep : baseDependencies.get(comp(Reflect.get(base, "name") + "Comp")))
	...
	protected void append(MethodSpec.Builder methBuilder, Iterable<ClassSymbol> defComps, Seq<MethodSymbol> entries, Seq<MethodSymbol> inserts, Seq<MethodSymbol> wraps, boolean writeBlock, VarSymbol setter) {
		...
		for (MethodSymbol m : entries) {
			...
			if (setter != null && !hasSet && (anno(m, MethodPriority.class) == null || anno(m, MethodPriority.class).value() == 0)) {
				...
				for (MethodSymbol e : insertComp)
					if (!anno(e, Insert.class).after()) methBuilder.addStatement("this.$L()", name(e));

				if (!isAny(m, Modifier.ABSTRACT, Modifier.NATIVE)) {
			...
			String desc = desc(m);
			if (!isAny(m, Modifier.ABSTRACT, Modifier.NATIVE) && methodBlocks.containsKey(desc)) {
	...
	public record EntityDefinition(String name, Builder builder, Symbol naming, @Nullable ClassSymbol extend,
	                               Seq<ClassSymbol> components, Seq<FieldSpec> fieldSpecs) {
			public EntityDefinition(String name, Builder builder, Symbol naming, ClassSymbol extend, Seq<ClassSymbol> components, Seq<FieldSpec> fieldSpecs) {
				this.name = name;
