...
public class EntityIO {
	...
	public void writeInterpolate(MethodSpec.Builder method, Seq<VariableElement> fields) {
		...
		for (VariableElement field : fields) {
			...
			st("$L = $L($T.$L($L, $L, alpha))", name, BaseProcessor.annotation(field, SyncField.class).clamped() ? "arc.math.Mathf.clamp" : "", BaseProcessor.className(Mathf.class), BaseProcessor.annotation(field, SyncField.class).value() ? "lerp" : "slerp", lastName, targetName);
		}
	...
	public void io(BaseProcessor proc, String type, String field) {
		...
		if (BaseProcessor.isPrimitive(type)) {
			...
			if (write) {
				...
				st(field + "$T.content.getByID($T.$L, read.s())", BaseProcessor.className(Vars.class), BaseProcessor.className(ContentType.class), BaseProcessor.simpleName(type).toLowerCase().replace("type", ""));
			}
