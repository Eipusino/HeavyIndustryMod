...
float wallDist(vec2 coord, vec2 v) {
	...
	float dist = 1.0;
	tile *= 0.125;
	if (texture(u_texture, tileuv + vec2(step.x, 0.0)).a < 0.9) {
		dist = min(dist, 1.0 - tile.x);
	}
	if (texture(u_texture, tileuv - vec2(step.x, 0.0)).a < 0.9) {
		dist = min(dist, tile.x);
	}
	if (texture(u_texture, tileuv - vec2(0.0, step.y)).a < 0.9) {
		dist = min(dist, tile.y);
	}
	if (texture(u_texture, tileuv + vec2(0.0, step.y)).a < 0.9) {
		dist = min(dist, 1.0 - tile.y);
	}
	if (texture(u_texture, tileuv + vec2(-step.x, -step.y)).a < 0.9) {
...
void main() {
	...
	if (az >= watertop) { //water top
		...
		wd = wd + sin(wd * 14.0 + btime * 60.0 + texture(u_noise, soffset + vec2(0.42,0.69)).r * 15.0) * 0.25;
		fragColor = vec4(col, 1.0);
		if (wd + (texture(u_noise, offset).r - 0.5) * 0.5 < 0.3) {
			fragColor.rgb = mix(fragColor.rgb,vec3(89.0 / 256.0, 106.0 / 256.0, 184.0 / 256.0), 0.5);
		}
		fragColor.rgb *= fade(bcoords.xy, v);
	} else {
